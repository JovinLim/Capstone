...
#Defining function to instantiate consultation room config 1
def conrm1(self, m_id, rotate):
    ...
    if counter < 30:
        pass
    valid = valid_cor(m_id)
    if valid is False:
        m_id = original_mid
    return m_id, clusterid, valid

#generate type 2 cluster of consultation room
...
#generate type 3 cluster of consultation rooms
...
#Either predetermined or measured
pt_dist = 1
...
for i in range(len(rc)):
    rec_yid.append(rc[i][0])
    rec_xid.append(rc[i][1])
cr_set.append([min(rec_yid),min(rec_xid)])
cr_set.append([min(rec_yid),max(rec_xid)+ptd])
cr_set.append([max(rec_yid)+ptd,max(rec_xid)+ptd])
cr_set.append([max(rec_yid)+ptd,min(rec_xid)])
